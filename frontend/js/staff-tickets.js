document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMessages(messages) {
        ...
        messagesEl.innerHTML = messages.map((m) => `
            <div class="p-2 rounded-3 ${m.from !== 'customer' ? 'bg-primary bg-opacity-10 align-self-end' : 'bg-light align-self-start'}" style="max-width: 80%;">
                <div class="small fw-bold mb-1">${m.from !== 'customer' ? 'Our Team' : currentUserName}</div>
                <div>${m.text}</div>
    ...
    document.querySelectorAll('[data-open-chat]').forEach((btn) => {
        btn.addEventListener('click', async () => {
            ...
            chatModal.show();
            try {
                const response = await fetch(`/staff/tickets/${currentTicketId}/messages`);
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`/staff/tickets/${currentTicketId}/message`, {
                method: 'POST',
